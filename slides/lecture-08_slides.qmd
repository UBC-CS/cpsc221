...
struct Node{
   LIT data;
   Node * next;
   Node(LIT nData, Node * next=NULL): data(nData) {}
};
```
...
`p = NULL;` &nbsp; Do you like this? \_\_\_\_\_\_\_\_\_\_

`q = NULL;` &nbsp; Do you like this? \_\_\_\_\_\_\_\_\_\_
:::
::::
...
`p = NULL;` -- **no, this is bad.** `p` was the only handle on that first
`new int`. Setting it to `NULL` abandons the memory without freeing it. Point
at the orange row that now has nothing aimed at it: that row is stuck there
until the program exits.

`q = NULL;` -- **yes, this is good.** It comes right after `delete q`, so `q`
is dangling. Setting it to `NULL` is exactly the right habit.
...
`NULL` is harmless, `*` on `NULL` is fatal.
:::
...
| **3** | [`pointers3.cpp`](code/lecture-08/pointers3.cpp) | a leak, and two ways to meet `NULL` |
...
3. The three failures, in order: the leak (overwrite the only pointer to a
   `new`'d int), `delete` on a `NULL` pointer (legal, does nothing), and
   dereferencing `NULL`. **The third one really does crash** -- exit 139,
